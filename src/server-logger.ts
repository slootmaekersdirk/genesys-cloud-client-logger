...
export class ServerLogger {
  ...
  constructor(logger: Logger) {
    ...
    if (!logger.config.url || !logger.config.appVersion) {
      const errMessage =
        "Missing `url` and/or `appVersion` config options to set up server logging. " +
        "Not sending logs to server for this logger instance";
      this.logger.error(
        errMessage,
        { providedConfig: logger.config },
        { skipServer: true }
      );
    ...
    this.debounceLogUploadTime =
      logger.config.uploadDebounceTime || DEFAULT_UPLOAD_DEBOUNCE;
    this.logUploader = getOrCreateLogUploader(
      logger.config.url,
      logger.config.debugMode
    );

    //DSL window.addEventListener('unload', this.sendAllLogsInstantly.bind(this));
  ...
  addLogToSend(logLevel: LogLevel, message: string, details?: any): void {
    ...
    if (traceMessageSize > MAX_LOG_SIZE) {
      ...
      if (newTrace === null) {
        this.logger.error(
          "truncated message is too large to send to server. not sending message",
          {
            originalTrace: trace,
    ...
    if (exceedsMaxLogSize) {
      ...
      this.debug("calling sendLogsToServer(true)", {
        logBuffer: this.logBuffer,
      });
    ...
    if (useNewBufferItem) {
      this.debug("`this.logBuffer` was empty. pushing new buffer item", {
        logBuffer: this.logBuffer,
        bufferItem,
      });
  ...
  sendAllLogsInstantly(): Promise<any>[] {
    ...
    return this.logUploader.sendEntireQueue().concat(
      /* this will send any items in the buffer still */
      this.logBuffer.map((item: ILogBufferItem) =>
        this.logUploader.postLogsToEndpointInstantly(
          this.convertToRequestParams(item.traces.reverse()),
          { saveOnFailure: true }
        )
      )
    );
  ...
  private async sendLogsToServer(immediate = false): Promise<any> {
    ...
    if (!immediate) {
      if (!this.debounceTimer) {
        this.debug(
          `sendLogsToServer() 'immediate' is false. setting up 'debounceTimer' to ${this.debounceLogUploadTime}ms`
        );
        ...
        this.debug(
          `sendLogsToServer() 'immediate' is false. 'debounceTimer' is already running`
        );
      }
    ...
    try {
      ...
      this.debug("calling logUploader.postLogsToEndpoint() with", {
        bufferItem,
        newLogBuffer: this.logBuffer,
      });
      await this.logUploader.postLogsToEndpoint(
        this.convertToRequestParams(bufferItem.traces.reverse())
      );
    } catch (err) {
      this.logger.emit("onError", err);
      this.logger.error("Error sending logs to server", err, {
        skipServer: true,
      });
      ...
      if (["401", "404"].includes(statusCode)) {
        this.debug(
          `received a ${statusCode} from logUploader. stopping logging to server`
        );
  ...
  private truncateLog(logLevel: LogLevel, log: ILogMessage): ITrace | null {
    ...
    const originalTraceSize = calculateLogMessageSize(
      this.convertToTrace(logLevel, log)
    );
    ...
    if (truncatedTraceSize <= MAX_LOG_SIZE) {
      this.logger.warn(
        "message too large to send to server. truncated log details",
        {
          originalLog: log,
    ...
    if (truncatedTraceSize <= MAX_LOG_SIZE) {
      this.logger.warn(
        "message too large to send to server. truncated log details & log message",
        {
          originalLog: log,
          truncatedLog: logCopy,
          originalTraceSize,
  ...
  private debug(message: string, details?: any): void {
    ...
    console.log(
      `%c [DEBUG:${this.logger.config.appName}] ${message}`,
      "color: #32a852",
      deepClone(details)
    );
  }
