...
const STATUS_CODES_TO_RETRY_IMMEDIATELY = [408, 429, 500, 503, 504];

const STATUS_CODES_TO_RETRY_LATER = [401];
...
export const getOrCreateLogUploader = (
  url: string,
  debugMode = false
): LogUploader => {
...
export class LogUploader {
  ...
  async postLogsToEndpointInstantly(
    requestParams: ISendLogRequest,
    opts?: { saveOnFailure: boolean }
  ): Promise<any> {
    this.debug('sending request instantly', {
      requestParams,
  ...
  saveRequestForLater(request: ISendLogRequest): void {
    ...
    /* DSL  window.localStorage.setItem(
      SAVED_REQUESTS_KEY,
  ...
  getSavedRequests(): ILogRequest[] | undefined {
    ...
    if (jsonStr) {
      try {
        ...
        console.error('Failed to parse saved messages, ignoring', {
          savedMessagesStr: jsonStr,
        });
      }
    }*/
    return {} as ILogRequest[];
  }
  ...
  resetSendQueue() {
    this.debug('reseting send queue without sending currently queued data', {
  ...
  private async sendNextQueuedLogToServer(): Promise<void> {
    ...
    queueItem.deferred.promise.finally(() => {
      this.debug(
        'queue item completed. removing from queue and resetting send queue',
        {
          queueItemRequestParams: queueItem.requestParams,
          updatedSendQueue: this.sendQueue.map((i) => i.requestParams),
        }
      );
    ...
    this.debug('sending logs to server', {
    ...
    return backOff(() => this.backoffFn(queueItem.requestParams), {
      retry: (err: AxiosError): boolean => {
        ...
        return (
          /* DSL navigator.onLine && */
          (status && STATUS_CODES_TO_RETRY_IMMEDIATELY.includes(status)) ||
          code === 'ERR_NETWORK'
        );
      },
      numOfAttempts: 10,
      ...
      .then((response: any) => {
        this.debug('successfully sent logs to server', {
          requestParams: queueItem.requestParams,
          response,
        });
  ...
  private handleBackoffError(queueItem: IQueueItem, error: any) {
    ...
    const isRetriableStatus =
      (status && STATUS_CODES_TO_RETRY_IMMEDIATELY.includes(status)) ||
      STATUS_CODES_TO_RETRY_LATER.includes(status);

    if (isRetriableStatus || error.code === 'ERR_NETWORK') {
      this.debug(
        'Failed to sends logs to the server, moving request to the end of the queue',
        { requestParams: queueItem.requestParams, error }
      );
      this.saveRequestForLater(queueItem.requestParams);
    } else {
      this.debug('ERROR sending logs to server', {
        requestParams: queueItem.requestParams,
        error,
      });
    }
  ...
  private async backoffFn(
    ...
    if (savedRequests) {
      //DSL window.localStorage.removeItem(SAVED_REQUESTS_KEY);
